...
class ABB<E extends Comparable<E>> {  // Árvore Binária de Busca 
    ...
    */
    ...
    public void emNivel() {
        ...
        public float mediaDasMulheres() {
        return mediaDasMulheres(raiz)[0]; // índice 0 = média calculada
    }

    // Retorna um array de float: [média, soma, quantidade]
    private float[] mediaDasMulheres(Node node) {
        float soma = 0;
        int cont = 0;

        if (node != null) {
            float[] esq = mediaDasMulheres(node.getFilhoEsquerdo());
            float[] dir = mediaDasMulheres(node.getFilhoDireito());

            Object valor = node.getValue();
            if (valor instanceof Aluno) {
                Aluno a = (Aluno) valor;
                if (a.getSexo() == 'F') {
                    soma += a.getMedia();
                    cont++;
        ...
        float media = (cont > 0) ? soma / cont : 0;
        return new float[]{media, soma, cont};
    }
    ...
    private boolean eliminar(Node node, Node paiRaiz, Object e) {  // remove um elemento da árvore, retorna true ou false
        ...
        if (node == null) {  // não achou o elemento, não existe (chegou em uma folha, ou árvore vazia)
            ...
            if (compara(e, node.getValue()) == 0) {  // se o nó a eliminar, node, foi encontrado (ele guarda o objeto e)
                aux = node;
                if (node.getFilhoEsquerdo() == null && node.getFilhoDireito() == null) {  // caso 1: se node não possui filhos, basta eliminar o nó
                ...
                } else if (node.getFilhoDireito() == null) {   // caso 2a: se node só tiver o filho esquerdo
                ...
                } else if (node.getFilhoEsquerdo() == null) {   // caso 2b: se node só tiver o filho direito                    
